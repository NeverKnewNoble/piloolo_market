...
# ************API TO CALL AFRICAN PRODUCTS IN USD******************
@frappe.whitelist(allow_guest=True)
def african_items(self=None, method=None):
    try:
        ...
        items_with_usd_prices = frappe.db.sql("""
            SELECT 
                item.item_name,
                item.image,
                item.custom_is_traditional_wear_african_print,
            ...
            WHERE 
                price.currency = 'USD'
                AND item.custom_is_traditional_wear_african_print = 1
...
#  http://127.0.0.1:8001/api/v2/method/piloolo_market.api.african_cloth.african_items
